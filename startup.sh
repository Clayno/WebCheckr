...
docker run --rm -it -v /var/run/docker.sock:/var/run/docker.sock -v \${PWD}:/webcheckr/shared --user \$(id -u):\$(id -g) --group-add \$(stat -c '%g' /var/run/docker.sock) --net webcheckr webcheckr \$@
